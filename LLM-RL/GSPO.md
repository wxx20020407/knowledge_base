...
$$
r_t(\theta) = \frac{\pi_\theta(y_t \mid x, y_{:t-1})}{\pi_{\theta_{\text{old}}}(y_t \mid x, y_{:t-1})}
$$
...
假设每个 token 的 ratio 有独立的噪声 epsilon，则整个序列的 ratio 近似为：

$$
\prod_{t=1}^{T} (1 + \epsilon_t) \approx 1 + \sum_{t=1}^{T} \epsilon_t + \text{高阶项}
$$

当序列长度 T 很大时（LLM 的推理链可能有几千个 token），连乘的方差正比于 T。这意味着：
...
- **T**：序列长度（token 数量）
- **y_t**：第 t 个 token
- **y_{:t-1}** = (y_1, ..., y_{t-1})：第 t 步之前的历史 token

### Policy 相关

- **pi_theta(y | x)**：当前策略（sequence-level 概率）
- **pi_{theta_old}(y | x)**：旧策略

token-level 概率：

$$
\pi_\theta(y_t \mid x, y_{:t-1})
$$

sequence-level 概率（token 概率的连乘）：

$$
\pi_\theta(y \mid x) = \prod_{t=1}^{T} \pi_\theta(y_t \mid x, y_{:t-1})
$$
...
$$
\frac{\pi_\theta(o_i \mid q)}{\pi_{\theta_{\text{old}}}(o_i \mid q)} = \prod_{t=1}^{T} \frac{\pi_\theta(o_{i,t} \mid q, o_{i,:t-1})}{\pi_{\theta_{\text{old}}}(o_{i,t} \mid q, o_{i,:t-1})}
$$
...
$$
R(\theta) = \prod_{t=1}^{T} \frac{\pi_\theta(y_t \mid x, y_{:t-1})}{\pi_{\theta_{\text{old}}}(y_t \mid x, y_{:t-1})}
$$
...
直接使用 R(theta) 仍然是问题：T 越大，R(theta) 的方差越大。GSPO 的核心创新是对 R(theta) 做长度归一化：
...
$$
\log \tilde{R}(\theta) = \frac{1}{T} \log R(\theta) = \frac{1}{T} \sum_{t=1}^{T} \log \frac{\pi_\theta(y_t \mid x, y_{:t-1})}{\pi_{\theta_{\text{old}}}(y_t \mid x, y_{:t-1})}
$$
...
- 原始 R(theta)：方差正比于 T（方差随长度线性增长）
- 归一化后 R_tilde(theta)：方差正比于 1/T * T = 1（方差与长度无关）
...
- PPO：token-level ratio r_t 乘以 token-level advantage A_t
- GSPO：sequence-level ratio R_tilde 乘以 sequence-level advantage A
...
$$
\tilde{R}_i(\theta) = R_i(\theta)^{1/T_i}
$$

是第 i 个序列的长度归一化 ratio。

$$
A_i = r_i - \bar{r}
$$

是 group relative advantage。
...
**直觉：** 梯度方向由 log pi_theta(y | x) 决定（"如何调整策略让这个输出更可能/更不可能"），梯度大小由 w(y) 控制（"调整多少"）。R_tilde(theta) 和 clipping 共同决定了步长。
...
即：**所有 token 共用同一个 sequence-level ratio R_tilde(theta)**，但保留 token 级别的反向传播。

**为什么这样设计？**

- 直接对 R_tilde(theta) * A 做反向传播，梯度只通过 R_tilde(theta) 流动
- 但 R_tilde(theta) 本身就是 token-level 概率的函数，所以梯度自然会分配到每个 token
...
$$
\log \tilde{R} = \frac{1}{T} \sum_{t=1}^T \log r_t
$$

平均化操作将方差除以 T，抵消了连乘带来的方差增长。
...
|---------|----------------|----------------|
| T = 100 | 正比于 100 | 正比于 1 |
| T = 1000 | 正比于 1000 | 正比于 1 |
| T = 5000 | 正比于 5000 | 正比于 1 |
...
GSPO 的 clipping 频率更低——因为 R_tilde(theta) 的方差小，ratio 不容易超出 [1-epsilon, 1+epsilon] 的范围。这意味着更多的梯度信号被"有效利用"了，而不是被 clipping 截断。
