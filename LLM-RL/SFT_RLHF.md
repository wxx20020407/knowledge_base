...
$$
\max \log P(x) = \max \sum_t \log P(x_t \mid x_{1:t-1})
$$
...
- `y`：模型输出的完整序列
- `y_t`：序列中第 `t` 个 token
- `y_{1:t-1}` = `(y_1, ..., y_{t-1})`：第 `t` 步之前的历史 token
- `T`：序列长度

### Policy

- `pi_theta(y | x)`：当前模型（policy），参数为 theta，给定 prompt x 生成序列 y 的概率
- `pi_SFT`：SFT 模型，作为 RL 阶段的初始化策略和 KL 参考
...
- `r_phi(x, y)`：奖励模型，输入 prompt 和回答，输出一个标量 reward
- `phi`：RM 的参数

### 数据

- `D_SFT`：人工示范数据（prompt + 高质量回答）
- `D_pref`：人类偏好排序数据（同一 prompt 的多个回答的排序）
...
`y_w` 是被偏好的（preferred）回答，`y_l` 是不被偏好的（less preferred）回答。通常 K 约为 4 到 9。
...
1. **优化目标**：让 `r_phi(x, y_w) - r_phi(x, y_l)` 大于 0，即好回答的 reward 高于差回答
...
- 当 delta_r 很大（正）时：sigma(-delta_r) 趋近 0，梯度很小，模型已经能区分，停止学习
- 当 delta_r 趋近 0 时：sigma(-delta_r) 趋近 0.5，梯度最大，模型还分不清，需要重点学习
- 当 delta_r 为负时：sigma(-delta_r) 趋近 1，梯度很大，模型完全搞反了，需要大幅修正
...
| 问题 | 说明 |
|------|------|
| 标注员偏好不等于真实价值 | RM 学到的是特定标注群体的偏好 |
| Reward hacking | 模型可能找到 RM 的漏洞，生成高 reward 但实际很差的回答 |
...
$$
r_t(\theta) = \frac{\pi_\theta(y_t \mid x, y_{1:t-1})}{\pi_{\theta_{\text{old}}}(y_t \mid x, y_{1:t-1})}
$$
...
$$
L_{\text{LM}} = -\mathbb{E}_{x \sim D_{\text{pretrain}}}[\log \pi_\theta(x)]
$$
...
RLHF 优化的本质是：提高高 reward 输出的概率，同时通过 KL 约束保持 pi_theta 约等于 pi_SFT。
...
| 模块 | 目标 | 学什么 |
|------|------|--------|
| SFT | 最大化 log pi(y* given x) | 怎么说话（格式、模式） |
| RM | 最大化 log sigma(rw - rl) | 什么是好话（偏好排序） |
| RLHF | 最大化 r - beta times DKL | 多说好话（重分配概率） |
