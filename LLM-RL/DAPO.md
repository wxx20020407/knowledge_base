...
- π_θ(o_i | q)：当前策略（sequence-level 概率）
- π_{θ_old}(o_i | q)：旧策略
...
$$w_{i,t} = \frac{\pi_\theta(o_{i,t} \mid q, o_{i,\lt t})}{\pi_{\theta_{\text{old}}}(o_{i,t} \mid q, o_{i,\lt t})}$$
...
GRPO 的 loss 中有 1/T_i，即对每个序列的 token loss 取平均。这意味着：

$$\text{每个序列对总 loss 的贡献} = \frac{1}{T_i} \sum_{t=1}^{T_i} L_t$$

无论序列长度如何，每个序列的总贡献是"被平均过的"。具体来说：

- 短序列（T_i = 100）：每个 token 的梯度权重为 1/100
- 长序列（T_i = 1000）：每个 token 的梯度权重为 1/1000

**后果：** 长序列中每个 token 的梯度被稀释了 10 倍。如果一个长回答是好回答（advantage 为正），模型从中学到的东西比一个短好回答少得多。这在长推理链（long chain-of-thought）的训练中尤为严重——模型倾向于生成短回答以获取更大的梯度信号。
...
例如 ε = 0.2，则 w ∈ [0.8, 1.2]。

问题：当 advantage 为正（好样本）时，w 不能超过 1+ε；当 advantage 为负（坏样本）时，w 不能低于 1-ε。上下界是对称的。
...
其中 ε_h 大于 ε_l。例如 ε_l = 0.1, ε_h = 0.3，则 w ∈ [0.9, 1.3]。
...
定义最大长度 T_max，直接丢弃超过长度的样本：

$$\text{o_i 被丢弃 if } T_i \gt T_{\max}$$

**本质：** 改变了 sampling distribution：

$$P(o \mid q) \rightarrow P(o \mid q, T \leq T_{\max})$$
...
- **简单 prompt**：大部分采样都能答对，σ 趋近于 0，学习信号很弱
- **困难 prompt**：只有少数采样答对，σ 大，学习信号强
...
具体实现：如果一个 prompt 的所有采样结果 reward 都相同（σ = 0），则跳过这个 prompt 或增加采样数；如果 reward 分化明显，保持或减少采样数。
...
| 维度 | GRPO | DAPO |
|------|------|------|
| Loss 归一化 | sequence-mean（1/T_i） | token-level（统一归一化） |
| Clipping | 对称 [1-ε, 1+ε] | 非对称 [1-ε_l, 1+ε_h] |
