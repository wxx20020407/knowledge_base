...
问题在于，每次更新后策略发生变化，之前采集的数据就"过期"了。如果用过期数据做大更新，策略可能跳到一个很差的区域，导致 reward 断崖式下跌——这就是所谓的 **catastrophic policy collapse**。
...
- 时刻 t 的状态（state），代表环境的观测
- 时刻 t 的动作（action），由策略采样得到
- 当前策略，输入状态输出动作的概率分布
- 上一轮更新前的旧策略
- 当前策略参数
- 更新前的策略参数（用于 importance sampling）
...
**直觉理解：** Advantage 回答的是"在某个状态下，采取某个动作比平均水平好多少？"

- 采取动作后的期望累积回报
- 在该状态下的平均期望回报（baseline）
- Advantage 为正：这个动作比平均好，应该增加其概率
- Advantage 为负：这个动作比平均差，应该降低其概率

减去 baseline 的好处是：不改变梯度的期望（无偏），但显著降低方差。
...
**直觉理解：** r_t 衡量新策略相对于旧策略，对某个动作的"偏好变化程度"。

- r_t = 1：新旧策略对这个动作的概率相同
- r_t 大于 1：新策略更倾向于这个动作
- r_t 小于 1：新策略更不倾向于这个动作

**为什么需要 importance ratio？** 因为数据是用旧策略采集的，但我们想优化新策略。Importance sampling 允许我们"修正"这个分布差异，从而复用旧数据。
...
这里 r_t 是 importance ratio。问题是：当 r_t 偏离 1 很多时（即新旧策略差异很大），importance sampling 的方差会急剧增大，导致梯度估计不准确，训练不稳定。

**直觉：** 想象你在一个山谷里，importance ratio 告诉你"新位置比旧位置陡了多少"。如果新旧位置差距太大，这个估计就不可靠了——可能把你引向悬崖。

### PPO Clipped Objective

PPO 的核心创新——用 clipping 限制 r_t 的范围：
...
- clip(x, a, b) = max(a, min(x, b))：将 x 限制在 [a, b] 区间内

### 核心机制：分情况理解

这个公式看似复杂，但可以分两种情况来理解：

**情况 1：Advantage 为正（好的动作，应该增加概率）**

此时 r_t * A_t 是正的，我们想最大化它。但 min 操作意味着：

$$
L = \min(r_t A_t, \; \text{clip}(r_t, 1-\epsilon, 1+\epsilon) \cdot A_t)
$$

当 r_t 大于 1 + epsilon 时，clip 后的值更小，min 取 clip 值，梯度被截断，不再鼓励进一步增大 r_t。

**直觉：** 即使某个动作很好，也不要过度增加它的概率——防止策略"过度自信"。

**情况 2：Advantage 为负（坏的动作，应该降低概率）**

此时 r_t * A_t 是负的，我们想最大化它（即让 r_t 更小）。但当 r_t 小于 1 - epsilon 时，clip 后的值更大（更接近 0），min 取原始值，梯度被截断。

**直觉：** 即使某个动作很差，也不要过度降低它的概率——保持一定的探索能力。

### 梯度视角

当 r_t 在 [1 - epsilon, 1 + epsilon] 范围内时，clipping 没有生效，梯度正常流动。

当 r_t 超出 [1 - epsilon, 1 + epsilon] 范围时，min 操作会选择 clip 后的值，此时梯度为 0——更新被"截断"。
...
| 维度 | TRPO | PPO-Clip |
|---|---|---|
| 约束方式 | 硬约束（KL 散度不超过 delta） | 软约束（clipping） |
...
直觉：TRPO 说"KL 散度不能超过某个阈值"，PPO-KL 说"KL 散度越大，惩罚越重"。两者在数学上是等价的（当 beta 和 delta 对应时），但 PPO-KL 的实现简单得多。
...
**为什么可行？** 因为 PPO 的 clipping 机制保证了即使 r_t 偏离 1，梯度也会被截断，不会产生过大的更新。这给了我们"安全地多次使用同一批数据"的空间。
...
- delta_t 衡量的是"实际获得的回报与预期回报之间的差距"
- gamma：discount factor，通常 0.99

**问题：** 单步 TD error 方差低但有 bias（因为 V 是近似的）；多步回报（如 Monte Carlo return）无 bias 但方差高。
...
其中 lambda 在 [0, 1] 范围内，是控制 bias-variance tradeoff 的超参数。

### lambda 的作用

- lambda = 0：纯 TD error，低方差、高 bias
- lambda = 1：等价于 Monte Carlo return，无 bias、高方差
...
用当前旧策略与环境交互，采集 trajectory 数据。

### Step 2：计算 value 和 advantage

用当前的 value function 计算 TD error 和 GAE advantage：
...
其中 value loss 是预测值与目标值之差的平方，H 是策略的熵（鼓励探索）。
...
旧策略参数被当前参数覆盖，然后回到 Step 1 采集新数据。
...
| 贡献 | 解决的问题 | 实现方式 |
|------|-----------|---------|
| Clipped Objective | policy 更新过大导致崩溃 | 限制 importance ratio 的范围 |
...
$$
r(\theta) = \prod_{t=1}^{T} \frac{\pi_\theta(y_t \mid x, y_{1:t-1})}{\pi_{\theta_{\text{old}}}(y_t \mid x, y_{1:t-1})}
$$
...
PPO 需要一个 value function 来计算 advantage。但在 LLM 中：
...
当一个序列的最终 reward 很高或很低时，很难判断是哪个 token 贡献了这个结果。PPO 的 token-level advantage 估计在 LLM 场景下不够精确。
